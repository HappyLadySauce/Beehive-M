...
$RpcFile = Join-Path $ProtoDir "beehive-m-user.proto"
...
# 在项目根目录执行，并用 -I proto 指定 proto_path，传相对路径 proto/beehive-m-user.proto
Push-Location $RootDir
try {
    goctl rpc protoc proto/beehive-m-user.proto -I proto --go_out=$OutPbBase --go-grpc_out=$OutPbBase --zrpc_out=$OutDir --style=goZero --client=true -m
} finally {
    Pop-Location
}